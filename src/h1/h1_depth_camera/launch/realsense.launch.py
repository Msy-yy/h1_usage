...
configurable_parameters = [{'name': 'camera_name',                  'default': 'd435i',          'description': 'camera unique name'},
                           {'name': 'camera_namespace',             'default': 'h1_unit_238',   'description': 'namespace for camera'},
...
def launch_setup(context, params, param_name_suffix=''):
    ...
    set_domain = SetEnvironmentVariable('ROS_DOMAIN_ID', ros_domain_id)
    ...
    camera_node = launch_ros.actions.Node(
        ...
        emulate_tty=True,
        remappings=[
            ('~/robot_description', '/h1_unit_238/robot_description'),
            ('joint_states', '/h1_unit_238/platform/joint_states'),
            ('dynamic_joint_states', '/h1_unit_238/platform/dynamic_joint_states'),
            ('/diagnostics', '/h1_unit_238/diagnostics'),
            ('/tf', '/h1_unit_238/tf'),
            ('/tf_static', '/h1_unit_238/tf_static'),
        ],
        # Alternative approach (instead of SetEnvironmentVariable):
        # env={'ROS_DOMAIN_ID': ros_domain_id},
    )

    # Order matters: set env first, then start node
    return [set_domain, camera_node]
    ...
    return LaunchDescription(
        declare_configurable_parameters(configurable_parameters) + [
            DeclareLaunchArgument(
                'ros_domain_id', default_value='10', description='DDS domain ID'),
