...
def generate_launch_description():
    ...
    domain_id_arg = DeclareLaunchArgument(
        "domain_id",
        default_value="10",
        description="ROS 2 DDS domain ID used to isolate participants (ROS_DOMAIN_ID)."
    )
    ...
    lidar_topic_name_arg = DeclareLaunchArgument(
        "lidar_topic_name", default_value=f'/{nsp}/sensors/points', description="Topic name to listen for PointCloud2 input from the LiDAR (for example '/ouster/points')")
    ...
    gnss_topic_name_arg = DeclareLaunchArgument(
        "gnss_topic_name", default_value="emlid/fix", description="Topic name to listen for NavSatFix input from a GNSS (for example '/gps')")
    ...
    imu_topic_name_arg = DeclareLaunchArgument(
        "imu_topic_name", default_value=f'/{nsp}/sensor/imu/data', description="Topic name to listen for Imu input (for example '/imu')")
    ...
    #
    tf_remaps = [('/initialpose', 'initialpose'),
                 ('/tf', 'tf'),
                 ('/tf_static', 'tf_static')]
