...
def generate_launch_description():
    """
    Generate launch description for running h1_lidar components separately each
    component will run in a separate process).
    """
    h1_control_domain_id = SetEnvironmentVariable('ROS_DOMAIN_ID', '10')
    
    h1_lidar_pkg_dir = get_package_share_directory('h1_lidar')
    default_params_file = \
        Path(h1_lidar_pkg_dir) / 'config' / 'ouster_params.yaml'
    ...
    ouster_ns_arg = DeclareLaunchArgument(
        'ouster_ns', default_value='h1_unit_238/sensors')
